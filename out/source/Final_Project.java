...
public void drawFeelingPage() {
  ...
  int mappedReadValue =  Math.round(readValue / 10.23f);
  // map readValue to value within 0-255
  ledValue =  PApplet.parseInt(255*readValue/1024);
...
class Button{
  ...
  public void display(){
    ...
    if (is_circle == false){
      ...
      textFont(font_title);
      textAlign(CENTER);
      text(label, x + w/2, y + (h/2)+0.3f*font_size);
    }
    else{
      circle(x, y, w); // note that w=2r
      fill(255);
      textAlign(CENTER);
      text(label, x, y+0.3f*font_size);
    }
  }
...
public void keyPressed() {
  ...
  public void settings() {  size(1000, 800);  smooth(8); }
